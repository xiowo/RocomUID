...
async def get_rocom_egg_name(bot: Bot, ev: Event):
    ...
    for pet_id, item in pet_list.items():
        ...
        if item['breeding']['weight_low'] <= float(weight) * 1000 and float(weight) * 1000 <= item['breeding']['weight_high']:
            find_flag = find_flag + 1
        if find_flag == 2 and item['name'] not in find_name_list:
            find_list.append(pet_id)
            find_name_list.append(item['name'])
    if len(find_list) == 0:
        mes = "暂时没有找到该精灵蛋的匹配信息"
    else:
        mes = f"【查找条件】 \n尺寸：{length}m 重量：{weight}kg\n类型【{egg_type}精灵蛋】\n该精灵蛋有可能出生的精灵为\n"
        shuling = 1
        for pet_id in find_list:
            icon_src = ROCOM_ICON_PATH / f'{pet_list[pet_id]["icon"]}.png'
            ...
            mes += f" [{pet_list[pet_id]['name']}](mqqapi://aio/inlinecmd?command=rc图鉴{pet_list[pet_id]['name']}&reply=false&enter=true)"
...
async def get_rocom_egg_info(bot: Bot, ev: Event):
    ...
    danzu_str1 = ''
    for danzu in group1:
        danzu_str1 += f"[{danzu}](mqqapi://aio/inlinecmd?command={P}查找精灵蛋组,{danzu}&reply=false&enter=true) "
    danzu_str2 = ''
    for danzu in group2:
        danzu_str2 += f"[{danzu}](mqqapi://aio/inlinecmd?command={P}查找精灵蛋组,{danzu}&reply=false&enter=true) "
    peizhong_flag = 0
    for item in group1:
        if item in group2:
            peizhong_flag = 1
    ...
    if peizhong_flag == 0:
        await bot.send(MessageSegment.markdown(f"{mes}\n双方没有相同的蛋组，无法进行配种哦"))
    else:
        await bot.send(MessageSegment.markdown(f"{mes}\n双方拥有相同的蛋组，可以进行配种哦"))
...
async def get_rocom_info_img(bot: Bot, ev: Event):
    ...
    if len(pet_info["evolution_list"]) > 1:
        mes += f"\n查询图鉴："
        for item in pet_info["evolution_list"]:
            if str(item['pet_id']) != str(pet_id):
                from_name = pet_list[str(item['pet_id'])].get('form','')
                mes += f"[{item['name']}](mqqapi://aio/inlinecmd?command=rc图鉴{pet_list[str(item['pet_id'])]['name']}{from_name}&reply=false&enter=true) "
    if len(pet_info["egg_group"]) > 0:
        mes += f"\n查询蛋组："
        for groupname in pet_info["egg_group"]:
        ...
        if len(find_cocom_list) > 750:
            ...
            for rocom_id in find_cocom_list:
                if pet_list[rocom_id]['name'] not in find_name_list:
                    from_name = pet_list[rocom_id].get('form','')
                    rocomname = f"{pet_list[rocom_id]['name']}{from_name}"
                    mes += f"[{pet_list[rocom_id]['name']}](mqqapi://aio/inlinecmd?command=rc图鉴{rocomname}&reply=false&enter=true)"
                    if shuling == 4:
                        shuling = 1
                        mes += '\n'
                    else:
